...
import {// assertWebGL2Context,
  assertArrayTypeMatch} from './webgl2-checks';
...
export class Buffer {
  ...
  static makeFrom(gl, object = {}) {
    let check = object instanceof Buffer;
  ...
  subData({
    ...
    // TODO: adding checks here

    // Note: When we are just creating and/or filling the buffer with data,
    // the target we use doesn't technically matter, so use ARRAY_BUFFER
    // https://www.opengl.org/wiki/Buffer_Object
    this.bind({target});
    gl.bufferSubData(target, dstOffset, data, srcOffset, length);
    this.unbind({target});
  ...
  copyDataToBuffer({
    ...
    const {gl} = this;
    assert(dst.data, 'Destination buffer must have data');
    // TODO: more checks here
    if (size === 0) {
      size = this.data.length;
    }
    this.bind({target: GL.COPY_READ_BUFFER, buffer: this});
    this.bind({target: GL.COPY_WRITE_BUFFER, buffer: dst});
  ...
  copyDataFromBuffer({
    ...
    const {gl} = this;
    assert(src.data, 'Source buffer must have data');
    // TODO: more checks here
    if (size === 0) {
      size = src.data.length;
    }
    this.bind({target: GL.COPY_READ_BUFFER, buffer: src});
    this.bind({target: GL.COPY_WRITE_BUFFER, buffer: this});
    gl.copyBufferSubData(GL.COPY_READ_BUFFER, GL.COPY_WRITE_BUFFER, readOffset, writeOffset, size);
  }
  ...
  getSubData({
    ...
    const {gl, target} = this;
    this.bind({target});
    gl.getBufferSubData(target, readOffset, this.data, writeOffset, length);
  }
   ...
   * @returns {Buffer} - Returns itself for chaining.
   */
  bind({target = this.target, buffer = this} = {}) {
    this.gl.bindBuffer(target, buffer.handle);
    return this;
  }

  unbind({target = this.target, buffer = this} = {}) {
